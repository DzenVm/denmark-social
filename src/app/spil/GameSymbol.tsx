export type ReelIcon = "seven" | "crown" | "star" | "diamond" | "bell";

export const REEL_ICONS: ReelIcon[] = ["seven", "crown", "star", "diamond", "bell"];

export const ICON_LABELS: Record<ReelIcon, string> = {
  seven: "Syv",
  crown: "Krone",
  star: "Stjerne",
  diamond: "Diamant",
...
export default function GameSymbol({ name }: { name: ReelIcon | "?" }) {
  if (name === "?") {
    return (
      ...
      {name === "crown" && (
        <>
          <path
            ...
            strokeLinejoin="round"
          />
          <circle cx="16" cy="37" r="5" fill={fill} />
          <circle cx="50" cy="25" r="5.5" fill={fill} />
          <circle cx="84" cy="37" r="5" fill={fill} />
        </>
      )}
      ...
      {name === "bell" && (
        <>
          <path
            ...
            strokeLinejoin="round"
          />
          <circle cx="50" cy="83" r="6" fill={fill} stroke={STROKE} strokeWidth="2" />
        </>
      )}
    </svg>
  );
